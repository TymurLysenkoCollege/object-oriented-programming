...
long long cController::encrypt(byte key[KEY_SIZE / 8], string src_filename, string dst_filename, eChipperMode cryptMode) {
  ...
  while (iSrcFile) {
    if (GetByteOfText(bTextBlock, iSrcFile)) {
      crypt.EncryptBlock(bTextBlock, key, 0);
...
long long cController::decrypt(byte key[KEY_SIZE / 8], string src_filename, string dst_filename) {
  ...
  while (!iSrcFile.eof()) {
    if (GetByteOfText(bTextBlock, iSrcFile)) {
      crypt.EncryptBlock(bTextBlock, key, 1);
